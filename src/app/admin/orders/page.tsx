...
export default async function AdminOrdersPage() {
  const { rows: orders } = await pool.query(
    `SELECT id, order_number, email, total, status, created_at, items, shipping_name, tracking_number FROM orders ORDER BY created_at DESC`,
  );
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg overflow-hidden">
        {!orders.length ? (
          <p className="text-sm text-[var(--muted)] p-8 text-center">No orders yet.</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)]">
                {["Order", "Customer", "Items", "Total", "Status", "Tracking", "Date", "Cancel"].map((h) => (
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {orders.map((order: any) => {
                ...
                return (
                  <tr key={order.id} className="hover:bg-[var(--bg)] transition-colors align-top">
                    <td className="px-5 py-4 font-mono text-[12px] text-[var(--fg)] whitespace-nowrap">{order.order_number ?? order.id.slice(0, 8)}</td>
                    <td className="px-5 py-4">
                      <p className="text-[13px] text-[var(--fg)]">{order.shipping_name ?? "—"}</p>
                      <p className="text-[11px] text-[var(--muted)] truncate max-w-[160px]">{order.email}</p>
                    </td>
                    <td className="px-5 py-4 text-[12px] text-[var(--muted)]">{items.length} item{items.length !== 1 ? "s" : ""}</td>
                    <td className="px-5 py-4 text-[13px] font-medium text-[var(--fg)] whitespace-nowrap">${Number(order.total).toFixed(2)}</td>
                    <td className="px-5 py-4">
                      <OrderStatusUpdater orderId={order.id} currentStatus={order.status} statuses={ORDER_STATUSES} statusStyles={STATUS_STYLES} />
                    </td>
                    <td className="px-5 py-4">
                      <OrderStatusUpdater orderId={order.id} currentStatus={order.status} trackingNumber={order.tracking_number ?? ""} isTrackingField statuses={ORDER_STATUSES} statusStyles={STATUS_STYLES} />
                    </td>
